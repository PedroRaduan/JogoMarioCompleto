...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Clique no botão para iniciar", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(gameConfig.status==="start" && estadoDeJogo == 'start' && nariz_X != '' && nariz_Y != '') {
    world_start.play();
...
function jumping(character){
	if( (nariz_Y < 200 &&character.live) || (touchIsDown&&character.live) ){
